...
Agents can use `handoffs: ["other_agent"]` to transfer control, `delegations: ["sub_agent"]` to spawn a child task, `routers: [{condition: "...", target: "..."}]` to route deterministically, `auto_route: true` for semantic swarm routing, or `spawns:` to let an agent dynamically create narrowly-scoped sub-agents at runtime. Workflows (`workflows:`) support `type: "sequential"` (default), `"parallel"`, and `"vote"` (fan-out-and-consensus). Mark risky tools `requires_approval: true` (or `required_approvers: [...]` for multi-person sign-off). Prefer a real `memory.type` (`sqlite`/`postgres`/`redis`) over the default in-process one for anything beyond a toy demo — and if ANY tool sets `requires_approval`/`required_approvers`, `memory.type` MUST be `sqlite` (or another persistent backend), not a preference but a hard requirement: the approval pause is only ever visible/resumable through a persisted session, so leaving memory at its in-process default silently makes that tool's approval un-approvable. Consider `model.guardrails` (`mask_pii`, `banned_words`) for user-facing agents.
...
The one exception to "don't guess the memory backend": if ANY tool anywhere sets `requires_approval: true` or `required_approvers`, `memory.type` MUST be a persistent backend (`sqlite` is the safe default; `postgres`/`redis` only if the blueprint clearly implies one) — never leave it at the schema's own in-process default. This isn't a preference to ask about, it's a hard technical requirement: a HITL pause is only ever visible in Monitor's session list, and only resumable via `/resume`, when that session's state can actually be looked up afterward — an in-process backend loses it the moment the request ends, so the approval would be requested but never actually approvable. Still ask (via `clarifications_needed`) if the blueprint separately implies a *different* persistence need you're unsure about (e.g. explicit production deployment) — just don't leave memory.type unset or at its in-process default when any tool requires approval.
...
4. If any tool sets `requires_approval: true` or `required_approvers`, `memory.type` MUST be `sqlite` (or another persistent backend) — never `sliding_window`/`buffer`, and never `memory: {{}}` left to default to one.
