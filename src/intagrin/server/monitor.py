...
2. Persistence: which memory backend (`sqlite` for local/dev, `postgres`/`redis` for production) — if the task needs cross-session memory or production durability, ask rather than defaulting to sqlite. Exception: if any tool you're adding sets `requires_approval`/`required_approvers`, `memory.type` MUST be a persistent backend (default to `sqlite` without asking) — a HITL pause is only ever visible in this dashboard's session list, and only resumable, when the session it paused actually persists; the in-process default silently makes that approval un-approvable, so this one isn't a preference to check with the user about.
