...
- **Keeps a drafted prompt in sync with the blueprint.** A prompt file this command drafted carries a small trailing `{# intagrin:blueprint-hash ... #}` marker recording which version of `blueprint.md` it came from. The next time you compile against a *changed* blueprint, any such prompt is redrafted and shown as a green/red diff — reviewed and confirmed exactly like the `ai.yaml` diff above — instead of silently going stale. A prompt with no marker (hand-written, or one you've deliberately edited that comment away) is never touched or reconsidered. Declining an offered update keeps your text as-is and just marks that blueprint version reviewed, so you're not asked the same question again until the blueprint changes further.
